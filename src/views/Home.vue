...
export default {
  ...
  setup() {
    ...
    const getIpInfo = async () => {
      try {
        const data = await axios.get(
          `https://geo.ipify.org/api/v1?apiKey=API_IP&ipAddress=${queryIp.value}`
        );
    ...
    onMounted(() => {
      ...
      leaflet
        .tileLayer(
          "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=API_MAXBOX",
          {
            attribution:
